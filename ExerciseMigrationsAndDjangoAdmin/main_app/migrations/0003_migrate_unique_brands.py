# Generated by Django 5.0.4 on 2024-07-01 18:34
...
def create_unique_brands(apps, schema_editor):
    ...
    unique_brands_names = shoe.objects.values_list('brand', flat=True).distinct()  # [Adidas, Nike]
    unique_brands.objects.bulk_create(
        unique_brands(brand=brand_name) for brand_name in unique_brands_names
    )


def reverse_unique_brands(apps, schema_editor):
    unique_brands = apps.get_model('main_app', 'UniqueBrands')
    unique_brands.objects.all().delete()  # Delete all records in UniqueBrands table
