...
# Import your models
from main_app.models import Product, Category, Customer, Order, OrderProduct, Employee, Department
...
def product_quantity_ordered():
    ...
    for product in total_products_ordered:
        result.append(f"Quantity ordered of {product.name}:"
                      f"{product.total_ordered_quantity}")
    return "\n".join(result)


def select_employee(emp_id):
    selected_employee = Employee.objects.select_related('department').get(pk=emp_id)
...
def get_departments_with_employees():
    departments_with_employees = Department.objects.prefetch_related('employees').all()
...
total_products_ordered = Product.objects.annotate(Max(total_ordered_quantity='orderproduct__quantity'))
result = []
for product in total_products_ordered:
    result.append(f"Quantity ordered of {product.name}:"
                  f"{product.total_ordered_quantity}")
